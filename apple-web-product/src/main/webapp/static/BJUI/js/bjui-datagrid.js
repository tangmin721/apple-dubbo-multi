                ...
                $.each(datas, function(i, trData) {
                    ...
                    $.each(model, function(j, n) {
                        ...
                        if (n.calc) {
                            ...
                            if (n.calc == 'max') n.calc_max = n.calc_max ? (n.calc_max < number ? number : n.calc_max) : number;
...
if (that.col_showhide_count == 1) $showhide.find('> li.datagrid-col-check').addClass('disable');
                                ...
                                $showhide.find('> li.disable').removeClass('disable');
        ...
        $trs
            ...
            .on('bjui.datagrid.tr.delete', function(e) {
                ...
                if (!that.isDom) {
                    ...
                    that.data = data.remove(index);

                            // remove data in the current page data
                    that.allData = allData.remove(gridIndex);

                     // remove data in allData
                }
...
if (columnModel == that.editBtnsColumn) return;

         // edit btn column
        else
        if (columnModel.index == that.columnModel.length - 1) return;
...
$lockTrs = that.$lockTbody.find('> tr');
    ...
    Datagrid.prototype.filterInThead = function() {
        ...
        $.each(columnModel, function(i, n) {
            ...
            if (n.type == 'boolean') $input = $(BJUI.doRegional('<select name="' + n.name + '" data-toggle="selectpicker"><option value="">#all#</option><option value="true">#true#</option><option value="false">#false#</option></select>', regional));
...
btnpaging = BJUI.doRegional(btnpaging.replaceAll('#pageCurrent#', paging.pageCurrent).replaceAll('#count#', paging.total + '/' + parseInt(pageCount || 0)), pr);
        ...
        var setPageSize = function(pageSize) {
            ...
            $.unique(selectPages).sort(function (a, b) {
                return a - b
            });
...
if ($tr.hasClass(that.classnames.tr_edit)) return false;
        ...
        if ($tr.data('validator')) $tr.validator('destroy');
        ...
        var onLoad = function($dialog) {
            ...
            var createForm = function(data, $form, $tr) {
                ...
                $.each(columnModel, function(i, n) {
                    ...
                    if (!$input.isTag('checkbox')) $input.attr('size', 30);
                    if (n.type == 'boolean') {
...
$tr = $tr_next;
        ...
        var dialog_options = $.extend({}, {
            ...
            onLoad: onLoad,
            onClose: onClose
        }, (typeof options.editDialogOp == 'object' && options.editDialogOp));
