...
var ids = [];
    ...
    // tangmin自定义一个选中checkbox触发查询，传入id,让页面上的一个id元素reload
    // =======================
    Bjuiajax.prototype.doAjaxChangeRecode = function(options) {
        var that = this, $element = that.$element, $target = options.target ? $(options.target) : null;
        options = $.extend({}, Bjuiajax.DEFAULTS, typeof options == 'object' && options);
        var todo = function() {
            if (!$target || !$target.length) {
            ...
            var hasSelectd = $element.hasClass('selected');
            console.log(hasSelectd);
            console.log($element.find(':checkbox[name='+ options.group +']').get(0));
            var $checkbox = $element.find(':checkbox[name='+ options.group +']').get(0);
...
var ids = [];
    ...
    //tangmin
     $(document).on('click.bjui.bjuiajax.data-api', '[data-toggle="doajaxchangerecode"]', function(e) {
         var $this = $(this);
         var options = $this.data();
         if (!options.url) options.url = $this.attr('href');
         Plugin.call($this, 'doAjaxChangeRecode', options);
         e.preventDefault()
    })
